...
- **`sync` and `complete delta` overwrite manual requirement status edits.** Both commands re-derive requirement status from coverage entries. If any coverage entry is not `verified` (e.g. `planned`), `_compute_status_from_coverage()` hits the mixed-statuses branch and sets the requirement to `in-progress`. Two upstream bugs: (1) `deferred` is not in `VALID_STATUSES` — use `planned` instead; (2) the status derivation doesn't distinguish "intentionally deferred" from "incomplete". Accept the `in-progress` result and the validator warning for now. Upstream fix pending.
- **Revision status `accepted` is invalid.** Valid statuses for revisions: `draft`, `in-progress`, `completed`, `pending`, `deferred`, `complete`. Use `completed` instead of `accepted`.
